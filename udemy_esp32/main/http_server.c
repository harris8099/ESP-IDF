...
static esp_err_t http_server_get_dht_sensor_readings_json_handler(httpd_req_t *req)
{
	...
	sprintf(dhtSensorJSON, "{\"temp\":\"%d\",\"humidity\":\"%d\"}",getTemperature(),getHumidity());
...
/*
	wifiConnectInfo.json handler updates the webpage with connection info.
	@param req HTTP request for which the uri needs to be handled.
	@return ESP_OK
*/
...
static esp_err_t http_server_disconnect_json_handler(httpd_req_t *req)
{
...
/*
	localTime.json handler responds by sending sending the local time.
	@param req HTTP request for which the uri needs to be handled.
	@return ESP_OK
*/
...
{
	ESP_LOGI(TAG, "/ap_ssid_json requested");
...
static httpd_handle_t http_server_configure(void)
{
	...
	if (httpd_start(&http_server_handle, &config) == ESP_OK)
	{
		...
		httpd_uri_t wifi_disconnect_json = {
				...
				.method = HTTP_DELETE,
				.handler = http_server_disconnect_json_handler,
				.user_ctx = NULL
		};
